...
static int guest_namespace_capabilities[DARLING_GUEST_NAMESPACE_DESCRIPTOR_COUNT] = {
...
int __mldr_guest_namespace_capability_fd(unsigned int index) {
	if (index >= DARLING_GUEST_NAMESPACE_DESCRIPTOR_COUNT)
		return -1;
	return fcntl(guest_namespace_capabilities[index], F_GETFD) >= 0
		? guest_namespace_capabilities[index] : -1;
}
...
static void receive_guest_namespace_bootstrap(void) {
	if (guest_namespace_capabilities_present())
		return; // authenticated contents are revalidated by libsystem_kernel.
...
#ifdef DARLING_LIFECYCLE_COHORT_V1
	// This happens before loading dyld or entering any guest instruction.  The
	// bootstrap selector is the trusted inherited SOCK_SEQPACKET capability,
	// never a guest-controlled environment entry.
